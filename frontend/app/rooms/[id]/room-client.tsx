...
import { demoRoom, isDemoMode } from "@/lib/demo";
...
export default function RoomClient({ id }: { id: string }) {
  ...
  useEffect(() => {
    if (isDemoMode) {
      setRoomState({
        ...demoRoom,
        name: id === demoRoom.code ? demoRoom.name : `Demo room ${id}`,
        code: id,
      });
      setIsConnected(true);
      return;
          ...
          setRoomState({
            ...
            users:
              data.data.members?.map(
                (m: { guestName?: string; user?: { username?: string }; isHost: boolean }) => ({
                  name: m.guestName || m.user?.username || "Anonymous",
                  isHost: m.isHost || false,
                })
              ) || [],
            restaurants:
              data.data.roomRestaurants?.map(
                (rr: { restaurant: { id: number; name: string; cuisine?: string; votes?: unknown[] } }) => ({
                  id: rr.restaurant.id,
    ...
    if (isDemoMode) {
      setRoomState((prev) => {
        ...
        return {
          ...prev,
          restaurants: prev.restaurants.map((r) =>
            r.id === restaurantId ? { ...r, votes: r.votes + (_isUpvote ? 1 : -1) } : r
          ),
        };
      });
      return;
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2 px-4 py-2 bg-green-50 text-green-600 rounded-xl">
                ...
                <span className="text-sm font-medium">
                  {isDemoMode ? "Demo" : isConnected ? "Live" : "Connecting..."}
                </span>
              </div>
            </div>
          </div>
